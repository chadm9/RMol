...
import re, sys
#This script summarizes the results of an RMol search
#A usage example is 'python rmolsum.py 250 > results', which will create a file 'results' that summarizes your search
...
def main():
    ...
    for i in range(len(unique_structures)):
        ...
        print "Geometry: "
        read_in = open(str(unique_structures[i][0]) + '.RMol.out', 'r')
        output = ''.join([lines for lines in read_in])
        output = output.replace('\n', '').replace(' ', '')
