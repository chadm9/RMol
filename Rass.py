# RMol v. 1.0
# Author: W. C. McKee
#This algorithm performs a random search for isomers of a given composition in set by the user in the 'RMolin' file
#A usage example is 'python RMol.py'.

import math, random

class Atom:

    def __init__(self, symbol, valency):
        self.symbol = symbol
        self.x = 0.0
        self.y = 0.0
        self.z = 0.0
...
def ReadInput(input):
    ...
    return atoms, atom_numbers, atoms_valencies, numb_of_structures

def InitializeAtoms(atoms, atom_numbers, atom_valencies):
    atom_list = []
    for i in range(len(atoms)):
        j = 1
        while j <= int(atom_numbers[i]):
            atom_list.append(Atom(atoms[i], int(atom_valencies[i])))
            j = j + 1
...
def main():
    ...
    read_in = open('RMol.in', 'r')
    input = read_in.readlines()
    atoms, atom_numbers, atoms_valencies, numb_of_structures = ReadInput(input)
    structures_written = 0

    while structures_written < numb_of_structures:
        structure_list = []
        atom_list = InitializeAtoms(atoms, atom_numbers, atoms_valencies)
        while len(atom_list) != 0:
            structure_list= SetCoordinates(structure_list, atom_list)

	output = open(str(structures_written + 1) + '.RMol', 'w')
        output.write("%mem=4gb\n%lindaworkers=LINDA\n") 
        output.write("# UPBEPBE/LANL2DZ Opt=maxcycles=50\n")
        output.write("\nTitle\n\n")
        output.write("0 2\n")
